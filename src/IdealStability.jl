...
function test_ideal_stability(Bp, Bt, dpdr, R0, r0, rb; m1ncap = 7, m0ncap=3, rs0=1e-5, nmax=8, del=1e-5, integrator_reltol=10^(-20), integrator_reltol_no_rs=1e-5, verbose=true, rs_verbose=false, verify_sols=false, case=0, ignore_Suydam=false, return_psi_small=false, kwargs...)
    ...
    for n in m1set
        ...
        if rs==-1 
            ...
            if !ideal_stability_out(Bp, Bt, dpdr, k, m, rs, rb, del; integrator_reltol=integrator_reltol, plot_solution=false, return_psi_small=return_psi_small)
                verbose && print("Equilibrium unstable to ideal mode n=$(n),m=$(m).\n")
                verbose && ideal_stability_out(Bp, Bt, dpdr, k, m, rs, rb, del; integrator_reltol=integrator_reltol, plot_solution=true)
                return false
            end
        end
...
function test_ideal_stability(Bp, Bt, dpdr, k, m, r0, rs, rb, rs0, nmax, del, q; integrator_reltol=10^(-20), verify_sols=false, case=0, return_psi_small=false, kwargs...)
    ...
    if !verify_sols && case == 0
        ...
        if m!=0
            ...
            if case==3 || !ideal_stability_out(Bp, Bt, dpdr, k, m, rs, rb, del; integrator_reltol=integrator_reltol, plot_solution=false)
                return ideal_stability_out(Bp, Bt, dpdr, k, m, rs, rb, del; integrator_reltol=integrator_reltol, plot_solution=true, return_psi_small=return_psi_small)
            end
        else
            k = k_(n,R0)
            solin = integrate_psi_in(Bp, Bt, dpdr, k, m, r0, rb; integrator_reltol=integrator_reltol, g=g_)
...
function ideal_stability_out(Bp, Bt, dpdr, k, m, rs, rb, del; integrator_reltol=1e-20, plot_solution=false, return_psi_small=false)
